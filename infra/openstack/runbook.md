# AtonixCorp Infrastructure Runbook
...
|--------------------------|---------------------------------------------------------------------|
| Founder / AtonixCorp Owner | Defines non-negotiable standards; approves PROD governance changes |
| Platform / Infra Lead    | Owns `infra/openstack-atonixcorp`; approves STAGE + PROD infra changes    |
| Application Leads        | Own `apps/atonixcorp-core`; approve STAGE + PROD app changes      |
...
2. Clone `infra/openstack-atonixcorp`, create a feature branch.
...
1. Developer pushes a code change to `apps/atonixcorp-core` as a Gerrit changeset.
...
## 7. How to add a new AtonixCorp service
...
To verify that all resources in a given environment carry correct AtonixCorp tags:

```bash
# Using OpenStack CLI
openstack server list --tags system=atonixcorp --format json | jq '.[].Name'
...
| 3     | App build + deploy pipelines for AtonixCorp services | App pipelines |
