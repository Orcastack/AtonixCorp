# AtonixCorp – OpenStack Infrastructure

This directory contains all Infrastructure as Code (IaC) and pipeline definitions
for operating AtonixCorp on OpenStack using Gerrit change control and Jenkins pipelines.

**No manual changes are permitted in any AtonixCorp environment beyond temporary DEV experiments.**
...
infra/gerrit/
├── projects/
│   ├── infra-openstack-atonixcorp.config  Access rules for infra/openstack-atonixcorp
│   ├── apps-atonixcorp-core.config Access rules for apps/atonixcorp-core
...
1. Identify the last known-good commit in `infra/openstack-atonixcorp`.
