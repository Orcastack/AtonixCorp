...
const baselinePath = path.join(rootDir, 'atonixcorp-public-standards-baseline.json');
...
if (writeBaseline) {
  ...
  console.log(`Wrote AtonixCorp public standards baseline with ${sortedFindings.length} findings.`);
  process.exit(0);
}

if (!fs.existsSync(baselinePath)) {
  console.error('AtonixCorp public standards baseline is missing. Run npm run standards:baseline first.');
  process.exit(1);
}
...
if (newFindings.length === 0) {
  console.log(`AtonixCorp public standards check passed. Baseline findings tracked: ${baselineKeys.size}.`);
  process.exit(0);
}

console.error(`AtonixCorp public standards check failed with ${newFindings.length} new finding(s):`);
